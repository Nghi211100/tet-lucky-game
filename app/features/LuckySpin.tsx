...
const LuckySpin = () => {
  ...
  const muteBg = async () => {
    ...
    try {
      audio.muted = !muted
      setMuted(!muted)
      if(audio.paused || !muted) await audio.play()
    } catch (e) {
  ...
  return (
    <div className={`w-full h-full md:flex justify-center items-center py-10 relative space-y-6 z-0 ${openNote&&'h-screen overflow-hidden'}`}>
      ...
      <Note openNote={openNote} setOpenNote={setOpenNote} playBg={muted?muteBg:()=>{}}/>
